...
## PROMPT 001 — Repo Structure & Python Environment
...
## PROMPT 004 — Environment & Config Loader
...
## PROMPT 005 — FastAPI App Skeleton + Health Endpoint
...
## PROMPT 006 — Database Connection Pool
...
## PROMPT 007 — Fernet Encryption Service
...
## PROMPT 008 — TWA HMAC Middleware
...
Rules:
- SAME security rules as PROMPT 051: hmac.compare_digest mandatory.
...
## PROMPT 009 — Telegram Webhook Handler + Idempotency
...
4. Rate limit check (stub for now — returns True always, implemented in PROMPT 013).
...
## PROMPT 010 — Users Table: Upsert on /start
...
## PROMPT 012 — Upstash Redis Client Wrapper
...
Used exclusively by the rate limiter (PROMPT 013) and task queue.
...
## PROMPT 013 — Redis Sliding Window Rate Limiter
...
## PROMPT 014 — Dead Letter Queue Writer
...
## PROMPT 015 — Bot Command System: /help, /list, /delete, /stats
...
## PROMPT 016 — Logout + Session Refresh
...
## PROMPT 017 — Text Ingestion + Task Worker Loop
...
5. Routes by content_type to the correct ingester:
   ...
   - "url"  → url_ingester.process(task) [from PROMPT 019]
...
## PROMPT 018 — Voice Note Ingestion
...
## PROMPT 019 — URL Ingestion: Scraping + Save
...
- raw_text MUST be Fernet-encrypted — use services/encryption.py from PROMPT 007.
...
## PROMPT 022 — YouTube URL Pipeline
...
## PROMPT 023 — Modal Whisper Endpoint (Tier 0 STT)
...
## PROMPT 025 — Content Deduplication
...
## PROMPT 026 — Embedding Pipeline Integration
...
## PROMPT 027 — Redis Queue Monitoring + Dead Letter Retries
...
## PROMPT 028 — GET /api/items Endpoint
...
## PROMPT 029 — DELETE /api/items/{id} + IDOR Protection
...
## PROMPT 030 — Hybrid Search: Vector + Trigram
...
## PROMPT 031 — PDF Chunking + Multi-Chunk Embedding

**Skills:** `python-pro` · `postgres-best-practices`

```
Upgrade pdf_ingester.py (from PROMPT 021, depends on AI Cascade from PROMPT 025) to store per-chunk embeddings.
...
## PROMPT 032 — /api/search REST Endpoint + Auth Guard
...
## PROMPT 033 — Tag System: Auto-Generate + Filter
...
GET /api/items?tag=<tagname> (extend PROMPT 027):
  WHERE $3 = ANY(tags)
...
## PROMPT 034 — Search Result Ranking: Map-Reduce RAG
...
## PROMPT 036 — GET /api/graph Optimisation + Edge Pruning

**Skills:** `postgres-best-practices` · `postgresql-optimization` · `python-fastapi-development`

```
Optimise the GET /api/graph endpoint from PROMPT 033 for performance.
...
## PROMPT 037 — React + Vite Project Setup
...
## PROMPT 038 — Dashboard Layout + Header
...
## PROMPT 039 — Items Feed View (Alternative to Graph)
...
Clicking a card opens the NodePanel from PROMPT 048 (same component — reuse).
...
## PROMPT 040 — Empty States + Loading Skeletons
...
## PROMPT 041 — Toast Notification System
...
## PROMPT 042 — Mobile Responsive Layouts
...
## PROMPT 043 — Error Boundary + Network Error Handling
...
## PROMPT 044 — Keyboard Shortcuts + Accessibility
...
## PROMPT 045 — Progressive Web App (PWA) Configuration
...
## PROMPT 046 — Settings Page (Timezone, Preferences)
...
## PROMPT 047 — Force-Directed Canvas Renderer
...
## PROMPT 048 — Louvain Clustering + Hub Nodes in Canvas
...
Backend (full implementation — this prompt owns the louvain_clustering job):
...
## PROMPT 049 — Map View: Semantic Hubs + Louvain Visualisation
...
## PROMPT 050 — Node Side Panel Component
...
## PROMPT 051 — WebSocket Connection Status UI
...
## PROMPT 052 — WebSocket Real-Time Graph Updates
...
## PROMPT 053 — Telegram Login Widget + JWT Issuance
...
## PROMPT 054 — Login Page / Landing Page
...
## PROMPT 056 — Frontend WebSocket Hook + Real-Time Node Addition
...
## PROMPT 057 — Drive Connect UI on Website
...
## PROMPT 058 — Disconnect Drive Endpoint + Google Token Revocation
...
## PROMPT 059 — SM-2 Algorithm + Quiz Endpoints
...
## PROMPT 060 — Bot Inline Keyboard Quiz Flow
...
## PROMPT 061 — Quiz History + Performance Tracking
...
## PROMPT 062 — Streak Visualisation in Frontend
...
## PROMPT 063 — APScheduler Setup + All 5 Jobs
...
Job 2 — louvain_clustering (daily 02:00 UTC):
  Call run_louvain_for_all_users() — the full implementation lives in PROMPT 047
  (backend/scheduler/jobs/louvain_job.py). Do not re-implement here; import and call it.
...
## PROMPT 064 — Streak Counter + Drive Nudge Logic
...
## PROMPT 065 — Daily Digest Bot Message
...
## PROMPT 066 — Bot /remind Command + Natural Language Time Parsing
...
## PROMPT 067 — Reminder UI on Website
...
Reminder creation from NodePanel (PROMPT 048):
...
## PROMPT 068 — API Rate Limit for Web Endpoints
...
## PROMPT 070 — Batch Items Export (GDPR / Data Portability)
...
## PROMPT 071 — Fernet Key Rotation Script
...
## PROMPT 072 — Chrome Extension: Manifest V3 + Popup
...
## PROMPT 073 — Chrome Extension: Background Service Worker
...
## PROMPT 074 — Chrome Extension: Options Page
...
```
Improve image ingestion (PROMPT 020) with pre-processing for better OCR quality.
...
- Security scan runs in CI (from PROMPT 089) — must pass cleanly.
...
4. On dashboard load: if window.Telegram.WebApp.initDataUnsafe.user exists:
   → Send initData to backend for TWA auth (HMAC validation from PROMPT 008)
...
- Add INTERNAL_API_KEY to the env vars table (added in PROMPT 026).
- Add quiz_answers table to BACKEND_SCHEMA.md (added in PROMPT 059).
- Add item_chunks table to BACKEND_SCHEMA.md (added in PROMPT 069).
- Add content_hash column to items table description (added in PROMPT 024).

Update SCHEDULER.md:
- Add Job 6 — daily_digest_sender (added in PROMPT 063).
...
[ ] Smoke test (PROMPT 086) passes in production
[ ] README.md complete and accurate
[ ] All 100 prompts in this document have been executed
```
...
|---|---|---|---|
| 001 | Repo Structure & Python Environment | PHASE 0 | ☐ |
| 002 | Local Development Environment | PHASE 0 | ☐ |
| 003 | Neon Database DDL | PHASE 0 | ☐ |
| 004 | Environment & Config Loader | PHASE 0 | ☐ |
| 005 | FastAPI App Skeleton + Health Endpoint | PHASE 0 | ☐ |
| 006 | Database Connection Pool | PHASE 0 | ☐ |
| 007 | Fernet Encryption Service | PHASE 0 | ☐ |
| 008 | TWA HMAC Middleware | PHASE 1 | ☐ |
| 009 | Telegram Webhook Handler + Idempotency | PHASE 1 | ☐ |
| 010 | Users Table: Upsert on /start | PHASE 1 | ☐ |
| 011 | OpenAPI Spec + API Documentation | PHASE 1 | ☐ |
| 012 | Upstash Redis Client Wrapper | PHASE 1 | ☐ |
| 013 | Redis Sliding Window Rate Limiter | PHASE 1 | ☐ |
| 014 | Dead Letter Queue Writer | PHASE 1 | ☐ |
| 015 | Bot Command System: /help, /list, /delete, /stats | PHASE 1 | ☐ |
| 016 | Logout + Session Refresh | PHASE 1 | ☐ |
| 017 | Text Ingestion + Task Worker Loop | PHASE 2 | ☐ |
| 018 | Voice Note Ingestion | PHASE 2 | ☐ |
| 019 | URL Ingestion: Scraping + Save | PHASE 2 | ☐ |
| 020 | Image Ingestion | PHASE 2 | ☐ |
| 021 | PDF Ingestion | PHASE 2 | ☐ |
| 022 | YouTube URL Pipeline | PHASE 2 | ☐ |
| 023 | Modal Whisper Endpoint (Tier 0 STT) | PHASE 2 | ☐ |
| 024 | AI Cascade Service | PHASE 2 | ☐ |
| 025 | Content Deduplication | PHASE 2 | ☐ |
| 026 | Embedding Pipeline Integration | PHASE 2 | ☐ |
| 027 | Redis Queue Monitoring + Dead Letter Retries | PHASE 2 | ☐ |
| 028 | GET /api/items Endpoint | PHASE 3 | ☐ |
| 029 | DELETE /api/items/{id} + IDOR Protection | PHASE 3 | ☐ |
| 030 | Hybrid Search: Vector + Trigram | PHASE 3 | ☐ |
| 031 | PDF Chunking + Multi-Chunk Embedding | PHASE 3 | ☐ |
| 032 | /api/search REST Endpoint + Auth Guard | PHASE 3 | ☐ |
| 033 | Tag System: Auto-Generate + Filter | PHASE 3 | ☐ |
| 034 | Search Result Ranking: Map-Reduce RAG | PHASE 3 | ☐ |
| 035 | Graph API Endpoint | PHASE 3 | ☐ |
| 036 | GET /api/graph Optimisation + Edge Pruning | PHASE 3 | ☐ |
| 037 | React + Vite Project Setup | PHASE 4 | ☐ |
| 038 | Dashboard Layout + Header | PHASE 4 | ☐ |
| 039 | Items Feed View (Alternative to Graph) | PHASE 4 | ☐ |
| 040 | Empty States + Loading Skeletons | PHASE 4 | ☐ |
| 041 | Toast Notification System | PHASE 4 | ☐ |
| 042 | Mobile Responsive Layouts | PHASE 4 | ☐ |
| 043 | Error Boundary + Network Error Handling | PHASE 4 | ☐ |
| 044 | Keyboard Shortcuts + Accessibility | PHASE 4 | ☐ |
| 045 | Progressive Web App (PWA) Configuration | PHASE 4 | ☐ |
| 046 | Settings Page (Timezone, Preferences) | PHASE 4 | ☐ |
| 047 | Force-Directed Canvas Renderer | PHASE 5 | ☐ |
| 048 | Louvain Clustering + Hub Nodes in Canvas | PHASE 5 | ☐ |
| 049 | Map View: Semantic Hubs + Louvain Visualisation | PHASE 5 | ☐ |
| 050 | Node Side Panel Component | PHASE 5 | ☐ |
| 051 | WebSocket Connection Status UI | PHASE 5 | ☐ |
| 052 | WebSocket Real-Time Graph Updates | PHASE 5 | ☐ |
| 053 | Telegram Login Widget + JWT Issuance | PHASE 6 | ☐ |
| 054 | Login Page / Landing Page | PHASE 6 | ☐ |
| 055 | Google OAuth Flow | PHASE 6 | ☐ |
| 056 | Frontend WebSocket Hook + Real-Time Node Addition | PHASE 6 | ☐ |
| 057 | Drive Connect UI on Website | PHASE 6 | ☐ |
| 058 | Disconnect Drive Endpoint + Google Token Revocation | PHASE 6 | ☐ |
| 059 | SM-2 Algorithm + Quiz Endpoints | PHASE 7 | ☐ |
| 060 | Bot Inline Keyboard Quiz Flow | PHASE 7 | ☐ |
| 061 | Quiz History + Performance Tracking | PHASE 7 | ☐ |
| 062 | Streak Visualisation in Frontend | PHASE 7 | ☐ |
| 063 | APScheduler Setup + All 5 Jobs | PHASE 8 | ☐ |
| 064 | Streak Counter + Drive Nudge Logic | PHASE 8 | ☐ |
| 065 | Daily Digest Bot Message | PHASE 8 | ☐ |
| 066 | Bot /remind Command + Natural Language Time Parsing | PHASE 8 | ☐ |
| 067 | Reminder UI on Website | PHASE 8 | ☐ |
| 068 | API Rate Limit for Web Endpoints | PHASE 8 | ☐ |
| 069 | Drive Sync Service | PHASE 9 | ☐ |
| 070 | Batch Items Export (GDPR / Data Portability) | PHASE 9 | ☐ |
| 071 | Fernet Key Rotation Script | PHASE 9 | ☐ |
| 072 | Chrome Extension: Manifest V3 + Popup | PHASE 9 | ☐ |
| 073 | Chrome Extension: Background Service Worker | PHASE 9 | ☐ |
| 074 | Chrome Extension: Options Page | PHASE 9 | ☐ |
| 075 | Backend Test Suite: Full Coverage | PHASE 10 | ☐ |
| 076 | Frontend Test Suite: Vitest | PHASE 10 | ☐ |
| 077 | Load Testing with k6 | PHASE 10 | ☐ |
| 078 | Integration Test: Full Item Save Flow | PHASE 10 | ☐ |
| 079 | Security Penetration Tests: IDOR + Injection | PHASE 10 | ☐ |
| 080 | Performance Profiling: Vector Search Benchmarks | PHASE 10 | ☐ |
| 081 | End-to-End Test: Auth Flows | PHASE 10 | ☐ |
| 082 | Rate Limit Testing: Redis Pipeline Atomicity | PHASE 10 | ☐ |
| 083 | Image OCR Quality + Preprocessing | PHASE 10 | ☐ |
| 084 | Security Scanning: SAST + Dependency Audit | PHASE 10 | ☐ |
| 085 | Performance Testing: Frontend Bundle Optimisation | PHASE 10 | ☐ |
| 086 | Smoke Test Script (Production Verification) | PHASE 10 | ☐ |
| 087 | Security Audit Pass | PHASE 11 | ☐ |
| 088 | Pre-Deployment Checklist Execution | PHASE 11 | ☐ |
| 089 | GitHub Actions CI Pipeline | PHASE 11 | ☐ |
| 090 | Production Go-Live Sequence | PHASE 11 | ☐ |
| 091 | Telegram TWA Registration | PHASE 11 | ☐ |
| 092 | Monitoring & Observability Setup | PHASE 11 | ☐ |
| 093 | Monitoring: Structured Logging + Alerts | PHASE 11 | ☐ |
...
| 099 | Partition Manager CLI Script | PHASE 11 | ☐ |
